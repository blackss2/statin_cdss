...
import (
	...
	"github.com/blackss2/utility/hotserver"
...
func main2() {
	session, err := gorethink.Connect(gorethink.ConnectOpts{
		Address: "tnicdev.co.kr:28015",
	})
	if err != nil {
		log.Fatalln(err.Error())
	}

	db := createDB(session, "trial_diary")

	/*
		st := NewStudyTable(session, db)
		study, err := st.Study(gConfig.StudyId)
		if err != nil {
			log.Fatalln(err.Error())
		}
	*/

	st := NewSubjectTable(session, db)
	tt := NewStackTable(session, db)
	vt := NewVisitTable(session, db)
	ft := NewFormTable(session, db)
	dt := NewDataTable(session, db)
	ht := NewHistoryTable(session, db)

	pPort := flag.String("p", "80", "port")
	flag.Parse()

	hotserver.MainWithPort((*pPort), func(port string) {
		WebPath, err := filepath.Abs("./webfile")
		if err != nil {
			log.Fatalln(err)
		}
		...
		r.NoRoute(func(c *gin.Context) {
			...
			if tp, has := templateHash[path]; has {
				...
				var subject *Subject
				if path != "/" {
					s, err := st.Subject("b18075b5-ba92-4353-9051-bb82279c6cff") //TEMP
					if err != nil {
						c.AbortWithError(500, err)
					}
					subject = s
				...
				switch path {
				case "/main":
					forms, err := FormWithCache(ft, gConfig.StudyId)
					if err != nil {
						c.AbortWithError(500, err)
						return
					}

					stacks, err := tt.List(subject.Id)
					if err != nil {
						c.AbortWithError(500, err)
					}
				...
				case "/list":
					...
					stack, err := tt.Stack(subject.Id, form.Id)
					if err != nil {
						c.AbortWithError(500, err)
					}
					if stack == nil {
						s, err := tt.Insert(subject.Id, form.Id, TNow, ActorId)
						if err != nil {
							c.AbortWithError(500, err)
						}
						stack = s
					...
					position := "1"
					visit, err := vt.Visit(stack.Id, position)
					if err != nil {
						c.AbortWithError(500, err)
					}
					if visit == nil {
						v, err := vt.Insert(stack.Id, position, TNow, ActorId)
						if err != nil {
							c.AbortWithError(500, err)
						}
						visit = v
				...
				case "/form":
					...
					dataList, err := dt.ListByRowindex(visit.Id, rowindex)
					if err != nil {
						c.AbortWithError(500, err)
					}
					...
					case "DELETE":
						if form.Type != "visit" {
							...
							for _, data := range dataList {
								history := ht.Retain(data, TNow, ActorId)
								historyList = append(historyList, history)
								DeleteIdList = append(DeleteIdList, data.Id)
							}
							if len(DeleteIdList) > 0 {
								err := dt.DeleteById("visit_id", visit.Id, DeleteIdList)
								if err != nil {
									c.AbortWithError(500, err)
									return
								}
